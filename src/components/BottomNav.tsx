...
export const BottomNav: React.FC<BottomNavProps> = ({ 
  ...
  const menuItems = [
    ...
    { 
      ...
      tag: 'LOGS',
      code: 'INVENTORY.LOGS',
      desc: 'Daftar kebutuhan hardware, software, dan estimasi anggaran.',
      accent: '#ff9f30'
    },
  ];
  ...
  const handleSelectFeature = (id: string, label: string) => {
    setActiveTab(id);
    setIsHubOpen(false);
    if (addSystemLog) {
      addSystemLog('MODULE LAUNCHED', `Modul "${label}" diaktifkan via Command Hub.`, 'info');
    }
  };

  return (
    <>
      {/* Interactive Command Center Hub Drawer (Bagan 5 Fitur Utama) */}
      {isHubOpen && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-[#0b1623]/85 backdrop-blur-md animate-fade-slide-up select-none p-3 sm:p-5">
          ...
          <div className="w-full max-w-2xl mx-auto bg-[#0b1623] border-2 border-[#ff9f30] shadow-[0_0_40px_rgba(255,159,48,0.3)] rounded-3xl p-5 space-y-4 relative overflow-hidden flex flex-col max-h-[85vh]">
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h3 className="font-bold text-xs tracking-widest text-[#f0f0f0] uppercase flex items-center gap-2">
                    // COMMAND HUB: BAGAN 5 MODULE FITUR
                  ...
                  <p className="text-[9px] text-[#8b9bb4] uppercase tracking-wider mt-0.5">
                    Sentuh salah satu bagan fitur di bawah untuk navigasi instan
                  </p>
                </div>
              </div>
              ...
              {menuItems.map((item, idx) => {
                ...
                return (
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => handleSelectFeature(item.id, item.label)}
                    className={`p-4 border text-left transition-all duration-200 cursor-pointer rounded-2xl relative group flex items-start space-x-3.5 ${
                      isCurrent
                  ...
                  >
                    ...
                    <div className={`p-3 border shrink-0 transition-all rounded-xl ${
                      isCurrent 
                        ? 'bg-[#ff9f30] text-[#0b1623] border-[#ff9f30] shadow-md' 
            ...
            <div className="border-t border-[#1c2b3a] pt-3 flex justify-between items-center text-[8px] font-mono text-[#8b9bb4]">
              <span className="flex items-center gap-1">
                <Zap className="w-3 h-3 text-[#ff9f30]" /> SWIPE OR TAP TO SWITCH MODULES
              </span>
              <span className="text-[#00ff9d] font-bold">F'CUBE CORE OS v1.2</span>
            </div>
          </div>
        </div>
      )}
      ...
      <nav 
        ...
        className="fixed md:absolute bottom-0 left-0 right-0 cyber-nav-dock cyber-nav-dock-responsive z-40 flex justify-around items-center h-20 px-2 select-none transition-all duration-300"
      >
        ...
        {/* Glowing Pull-Up Drawer Handle (Pull up for 5 Feature Hub) */}
        <button
          type="button"
          ...
          title="Tarik ke atas / Klik untuk Bagan 5 Fitur Utama"
        >
          <ChevronUp className={`w-3 h-3 transition-transform duration-300 ${isHubOpen ? 'rotate-180 text-white' : 'animate-bounce'}`} />
          <span>{isHubOpen ? 'TUTUP BAGAN FITUR' : 'PULL UP // BAGAN FITUR'}</span>
        </button>
