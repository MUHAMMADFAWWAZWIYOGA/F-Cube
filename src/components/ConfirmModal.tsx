...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0b1623]/80 backdrop-blur-xs transition-all duration-200">
      ...
      <div 
        className={`w-full max-w-md bg-[#0b1623] border-2 ${style.border} shadow-2xl rounded-2xl relative overflow-hidden flex flex-col p-5 space-y-4 animate-fade-slide-up`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Top Header Tag */}
        <div className="flex items-center justify-between border-b border-[#1c2b3a] pb-3">
          <div className="flex items-center space-x-2">
            <span className={`text-[9px] font-bold uppercase tracking-widest px-2.5 py-0.5 border border-[#1c2b3a] rounded-full ${style.badgeBg}`}>
              {style.tag}
            </span>
            <div className="w-1.5 h-1.5 bg-[#ff9f30] animate-ping rounded-full" />
          </div>
          <button
            onClick={onCancel}
            className="p-1 text-[#8b9bb4] hover:text-white hover:bg-[#1c2b3a] transition-colors rounded-full"
            title="Tutup (ESC)"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="flex items-start space-x-4 py-2">
          <div className="p-3 bg-[#1c2b3a]/40 border border-[#1c2b3a] shrink-0 rounded-xl">
            {style.icon}
          </div>
        ...
        {/* Warning bar notice */}
        <div className="border border-[#1c2b3a] bg-[#1c2b3a]/20 p-2.5 rounded-lg text-[8px] font-mono text-[#8b9bb4] flex justify-between items-center">
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            type="button"
            onClick={onCancel}
            className="py-2.5 px-3 border border-[#1c2b3a] bg-[#1c2b3a]/40 text-[#8b9bb4] hover:text-white hover:bg-[#1c2b3a] text-[10px] font-bold tracking-wider uppercase transition-all duration-200 cursor-pointer rounded-xl"
          >
            {cancelText}
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className={`py-2.5 px-3 border text-[10px] font-bold tracking-wider uppercase transition-all duration-200 cursor-pointer rounded-xl ${style.confirmBtn}`}
          >
            {confirmText}
          </button>
        </div>
